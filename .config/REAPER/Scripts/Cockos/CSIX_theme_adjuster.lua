-- Customized v6 Theme Adjuster for CSIX Themes / REAPER 7
-- Version: 3.10
...
function getCurrentPalette()
  return palette.idx[palette.current] or 'STRONG'
end
...
if reaper.MB('Assigns random colors to ALL tracks in the Project\nusing the selected Palette.\n\
Tracks which share a color will be given the\nsame new color.', "Recolor entire Project?", 1) == 1 then
...
local r = reaper
local verifysws = r.APIExists( 'CF_GetSWSVersion' )
...
if (gfx.mouse_cap == 4) then gradientstep = -10 --- Ctrl
  ...
  elseif (gfx.mouse_cap == 4+8) then gradientstep = 10 end --- Ctrl+Shift
...
local trackCount = r.CountSelectedTracks(0)
local gradientstep =  math.ceil((gradientstep*10)/(trackCount+1))
 for i = 0, trackCount - 2, 1 do
    ...
    r.SetTrackColor(nexttrack, newColorNative)
       if coloritem == true then
          --- r.Main_OnCommandEx(40421, 0, 0) --- do not apply to custom colored items
...
reaper.ShowMessageBox( "This function requires the SWS/S&M extension.\n\nThe SWS/S&M extension can be downloaded from www.sws-extension.org.\n\
https://forum.cockos.com/showthread.php?p=1840185", "ERROR", 0)
    end
end
...
if reaper.GetOS() == "OSX64" or reaper.GetOS() == "OSX32" or reaper.GetOS():match("^Win") == nil then
  ...
  gfx.setfont(5, "Roboto", 11) -- IMPORTANT : match the font and size (by eye, oops!) of TCP & EnvCP labels
  ...
  gfx.setfont(1, "Roboto", 12)
  gfx.setfont(2, "Roboto", 12)
  gfx.setfont(3, "Roboto", 16)
  gfx.setfont(4, "Roboto", 15) -- used in : undocked palette title
  gfx.setfont(5, "Roboto", 14) -- IMPORTANT : match the font and size (by eye, oops!) of TCP & EnvCP labels
  gfx.setfont(11, "Roboto", 26)
  gfx.setfont(12, "Roboto", 26)
  gfx.setfont(13, "Roboto", 28)
  gfx.setfont(14, "Roboto", 36) -- used in : undocked palette title
  gfx.setfont(15, "Roboto", 30)
end
...
function ParamRow:new(parent,valsTable,rowIdx)
  local o = {}
  self.__index = self
  if (rowIdx%2==0) then rowBgCol = {58,60,64} else rowBgCol = {0,0,0,35} end --- bg scheme color (theme -- fill rows global)
...
function Swatch:new(parent,o)
  ...
  local label = Element:new(o, {x=100-(palStrW/2),y=20,w=palStrW+10,h=18,text={str=palStr,style=2,align=9},color={50,51,55}})
  adoptChild(parent,o)
  setmetatable(o, self)
  return o
end
...
function paramIdxGet(param)
  ...
  if param == 'tcp_indent' or param == 'tcp_control_align' or param == 'tcp_customButton' or param == 'tcp_meterFlip' 
...
function themeCheck()
  ...
  if theme ~= oldTheme or theme == nil then
    ...
    if theme ~= 'CSIX' or theme_version < 1 then --theme_version catch for later changes
...
function switchTheme()
  local str = string.match(reaper.GetLastColorThemeFile(), '^(.*)[/\\].+$')
    if(reaper.file_exists(str.."/CSIX-BASIC_unpacked.ReaperTheme")==true) then
        openTheme = reaper.OpenColorThemeFile(str.."/CSIX-BASIC_unpacked.ReaperTheme")
    else if(reaper.file_exists(str.."/CSIX-BASIC.ReaperThemeZip")==true) then
            openTheme = reaper.OpenColorThemeFile(str.."/CSIX-BASIC.ReaperThemeZip")
            
        else if(reaper.file_exists(str.."/CSIX-BASIC-Narrow_unpacked.ReaperTheme")==true) then
                openTheme = reaper.OpenColorThemeFile(str.."/CSIX-BASIC-Narrow_unpacked.ReaperTheme")
            else if(reaper.file_exists(str.."/CSIX-BASIC-Narrow.ReaperThemeZip")==true) then
                    openTheme = reaper.OpenColorThemeFile(str.."/CSIX-BASIC-Narrow.ReaperThemeZip")
                    
                else if(reaper.file_exists(str.."/CSIX-BC-Extended_unpacked.ReaperTheme")==true) then
                        openTheme = reaper.OpenColorThemeFile(str.."/CSIX-BC-Extended_unpacked.ReaperTheme")
                    else if(reaper.file_exists(str.."/CSIX-BC-Extended.ReaperThemeZip")==true) then
                            openTheme = reaper.OpenColorThemeFile(str.."/CSIX-BC-Extended.ReaperThemeZip")
                            
                        else if(reaper.file_exists(str.."/CSIX-MC-Producer_unpacked.ReaperTheme")==true) then
                                openTheme = reaper.OpenColorThemeFile(str.."/CSIX-MC-Producer_unpacked.ReaperTheme")
                            else if(reaper.file_exists(str.."/CSIX-MC-Producer.ReaperThemeZip")==true) then
                                    openTheme = reaper.OpenColorThemeFile(str.."/CSIX-MC-Producer.ReaperThemeZip")
                                    
                                else if(reaper.file_exists(str.."/CSIX-DM-Xenon_unpacked.ReaperTheme")==true) then
                                        openTheme = reaper.OpenColorThemeFile(str.."/CSIX-DM-Xenon_unpacked.ReaperTheme")
                                    else if(reaper.file_exists(str.."/CSIX-DM-Xenon.ReaperThemeZip")==true) then
                                            openTheme = reaper.OpenColorThemeFile(str.."/CSIX-DM-Xenon.ReaperThemeZip")

                                        else if(reaper.file_exists(str.."/CSIX-AE-Narrow-dark_unpacked.ReaperTheme")==true) then
                                                openTheme = reaper.OpenColorThemeFile(str.."/CSIX-AE-Narrow-dark_unpacked.ReaperTheme")
                                            else if(reaper.file_exists(str.."/CSIX-AE-Narrow-dark.ReaperThemeZip")==true) then
                                                    openTheme = reaper.OpenColorThemeFile(str.."/CSIX-AE-Narrow-dark.ReaperThemeZip")
                                                else reaper.ShowConsoleMsg("CSIX theme not found")
                                                end
                                            end
                                        end
                                    end
...
function resetParams()
    if reaper.MB('DISCARD ALL settings for this theme and reload \nthe theme with DEFAULT values?', "Reset Theme Settings", 1) == 1 then
    for i=-1005,-1000,1 do
...
tcpFxListColumnVal = {'1 - 2','1 - 3','2 - 4','3 - 5','4 - 6',}
...
helpR_help = 'CSIX Theme Adjuster v.3.10'
...
helpR_totalReset = 'DISCARDS ALL (!) user settings for this theme and RELOADS the theme with DEFAULT values.'
...
helpR_buttonBrightness = 'Sets the label brightness on ALL action buttons globally. Requires REAPER 7.'
helpR_custOverlay = 'Adds an extra layer of custom color to all control panels (except on alternate full-tone layouts). Requires REAPER 7.'
...
helpR_showActBtnTcp = 'Allow action buttons to be displayed when the width of the selected panel is wider than the set limit to hide actions. '
                    ..'*The feature is currently setup to work only with CSIX Narrow themes and requires REAPER 7.'
...
helpR_showActBtnMcp = 'Allow action buttons to be displayed on selected tracks and master mixer panel (when size permits). '
                    ..'Exclamation mark (!) means enhanced appearance with the same setting. Requires REAPER 7.'
...
helpR_showIndentGuide = 'Displays folder indentation guides with a maximum depth of eight levels.  '
                    ..'*The feature is currently configured for themes in CSIX BC and AE Series. Requires REAPER 7.'
...
helpR_fxparmColumns = 'For CSIX AE Series, Layout C): Sets the number of COLUMNS if the right-aligned INSERTS/FX LIST has an overflow.  '
...
helpR_embed = 'When FX UI EMBEDDING is present, use this setting to expand with of '
            ..'the extended mixer when displayed as Sidebar.'
helpR_embedMast = 'If one of the alternate Master Mixer layouts: "Wide_Master" or "Wide_Sbar_Master"  '
                ..'is chosen, and FX UI EMBEDDING is present, use '
                ..'this setting to expand with of the extended mixer.'
...
helpR_colgradeChildren = 'Sets a color gradient for child tracks, based on selected Folder track color.    '
                   ..'Mouse modifiers (Shift, Ctrl or both) changes gradient.  '
                   ..'*This is a mod of a script made by ICio. Requires SWS/S&M extension.'
...
apply = {}
root = Element:new(nil, {x=0,y=0,drawx=0,drawy=0,w=_gfxw,h=_gfxh,color={58,60,64}}) -- 38,38,38 -- fill theme

_wrongTheme = Element:new(root, {flexW='100%',h=30,color={24,24,24,}})
Element:new(_wrongTheme, {x=6,y=2,w=26,h=26,img='icon_warning_on'})
_theme = Element:new(_wrongTheme, {x=32,y=2,w=100,h=15,text={str='',align = 4,col={180,180,181}}})
Element:new(_theme, {x=0,y=11,w=180,h=15,text={str='is not compatible with this script',align = 4,col={255,51,0}}})
_switchTheme = Button:new(_wrongTheme, {flow=true,img='button_empty',imgType=3,x=-6,y=0,flexW='100%',border='x',action=switchTheme,text={str='Switch to #CSIX', align=5,lineSpacing=12,col={255,255,255}}})
...
Element:new(_gammaBox, {x=0,y=4,w=80,h=11,text={str='GAMMA',style=2,align=4,col={180,180,181}}})
Readout:new(_gammaBox,{x=0,y=16,w=52,h=11,userEntry=true,action=doFader,moColor={60,60,60},param={-1000},text={str='',align=4,col={180,180,181}}})
...
Element:new(_saturationBox, {x=0,y=4,w=80,h=11,text={str='SATURATION',style=2,align=4,col={180,180,181}}})
Readout:new(_saturationBox,{x=0,y=16,w=52,h=11,userEntry=true,action=doFader,moColor={60,60,60},param={-1004},text={str='',align=4,col={180,180,181}}})
...
_applyBoxTcp = Element:new(_pageTrack, {flow=true,w=228,h=30, color={58,60,64}})
...
--- apply.tcp[2] = Button:new(_applyBoxTcp, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'tcp','150%_'}}) --- CSIX (scale not available)
...
Button:new(_pageMixer, {flow=true,w=45,img='layout_A_blue',imgType=3,border='x',action=doActiveLayout,param={'mcp','A'}})
Button:new(_pageMixer, {flow=true,w=43,img='layout_B_blue',imgType=3,action=doActiveLayout,param={'mcp','B'}})
Button:new(_pageMixer, {flow=true,w=45,img='layout_C_blue',imgType=3,action=doActiveLayout,param={'mcp','C'}})
_applyBoxMcp = Element:new(_pageMixer, {flow=true,w=228,h=30, color={58,60,64}})
...
--- apply.mcp[2] = Button:new(_applyBoxMcp, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'mcp','150%_'}}) --- CSIX (scale not available)
...
--- Button:new(_pageMixer, {flow=true,img='show_fx',imgType=3,border='x',action=actionToggle,param=40549}) --- CSIX - omitted
--- Button:new(_pageMixer, {flow=true,img='show_param',imgType=3,border='',action=actionToggle,param=40910}) --- CSIX - omitted
--- Button:new(_pageMixer, {flow=true,img='show_send',imgType=3,border='',action=actionToggle,param=40557}) --- CSIX - omitted
...
_applyBoxEnv = Element:new(_pageEnv, {flow=true,w=188,h=30, color={58,60,64}})
apply.envcp = {Button:new(_applyBoxEnv, {x=5,y=5,w=61,img='apply_100',imgType=3,action=applyLayout,param={'envcp',''}})}
--- apply.envcp[2] = Button:new(_applyBoxEnv, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'envcp','150%_'}}) --- CSIX (scale not available)
...
--- Button:new(_pageEnv, {flow=true,img='match_folder_indent',imgType=3,border='x',action=paramToggle,param='envcp_folder_indent'}) --- CSIX - omitted
...
_applyBoxTrans = Element:new(_pageTrans, {flow=true,w=188,h=30, color={58,60,64}})
...
--- apply.trans[2] = Button:new(_applyBoxTrans, {flow=true,w=61,img='apply_150',imgType=3,action=applyLayout,param={'trans','150%_'},helpL=helpL_applySize}) --- CSIX (scale not available)
...
--- Button:new(_pageTrans, {flow=true,img='show_play_rate',imgType=3,border='x',action=actionToggle,param=40531}) --- CSIX - omitted
--- Button:new(_pageTrans, {flow=true,img='center_transport',imgType=3,border='',action=actionToggle,param=40533}) --- CSIX - omitted
--- Button:new(_pageTrans, {flow=true,img='time_sig',imgType=3,border='',action=actionToggle,param=40680}) --- CSIX - omitted
--- Button:new(_pageTrans, {flow=true,img='next_prev',imgType=3,border='',action=actionToggle,param=40868}) --- CSIX - omitted
--- Button:new(_pageTrans, {flow=true,img='play_text',imgType=3,border='',action=actionToggle,param=40532}) --- CSIX - omitted
...
_unDpageSpin = Spinner:new(_pageContainer, {spinStyle='image',valsImage='page_titles',x=130,y=0,w=253,action=doPageSpin}) --- CSIX ,readoutParam={editPage}
Button:new(_pageContainer, {flow=false,x=483,y=0,img='dock',imgType=3,w=30,action=doDock,helpL=helpL_dock,helpR=helpR_dock})
Element:new(_pageContainer, {x=0,y=39,w=513,h=1,color={116,128,149,225}}) -- black title div
...
_colAdjBoxStroke = Element:new(_pageGlobal_und, {x=0,y=30,w=513,h=555,color={116,128,149,225}}) -- stroke
_colAdjBox = Element:new(_colAdjBoxStroke, {x=1,y=1,w=511,h=553,color={50,51,55},helpL=helpL_layout}) -- fill
...
Element:new(_colAdjBox, {x=29,y=90,w=80,h=11,text={str='GAMMA',style=2,align=4,col={180,180,181}},helpR=helpR_colAdj})
Readout:new(_colAdjBox,{x=105,y=88,w=52,h=15,userEntry=true,action=doFader,moColor={40,40,40},param={-1000},text={str='',align=4,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=29,y=188,w=80,h=11,text={str='HIGHLIGHTS',style=2,align=4,col={180,180,181}},helpR=helpR_colAdj})
Readout:new(_colAdjBox,{x=105,y=186,w=52,h=15,userEntry=true,action=doFader,moColor={40,40,40},param={-1003},text={str='',align=4,col={180,180,181}},helpR=helpR_colAdj})
Element:new(_colAdjBox, {x=192,y=188,w=80,h=11,text={str='MIDTONES',style=2,align=6,col={180,180,181}},helpR=helpR_colAdj})
Readout:new(_colAdjBox,{x=266,y=186,w=52,h=15,userEntry=true,action=doFader,moColor={40,40,40},param={-1002},text={str='',align=5,col={180,180,181}},helpR=helpR_colAdj})
Element:new(_colAdjBox, {x=358,y=188,w=80,h=11,text={str='SHADOWS',style=2,align=6,col={180,180,181}},helpR=helpR_colAdj})
Readout:new(_colAdjBox,{x=430,y=186,w=52,h=15,userEntry=true,action=doFader,moColor={40,40,40},param={-1001},text={str='',align=6,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=29,y=252,w=80,h=11,text={str='SATURATION',style=2,align=4,col={180,180,181}},helpR=helpR_colAdj})
Readout:new(_colAdjBox,{x=105,y=250,w=62,h=15,userEntry=true,action=doFader,moColor={40,40,40},param={-1004},text={str='',align=4,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=29,y=316,w=80,h=11,text={str='TINT',style=2,align=4,col={180,180,181}},helpR=helpR_colAdj})
Readout:new(_colAdjBox,{x=105,y=314,w=62,h=15,userEntry=true,action=doFader,moColor={40,40,40},param={-1005},text={str='',align=4,col={180,180,181}},helpR=helpR_colAdj})

Button:new(_colAdjBox, {x=60,y=336,w=30,img='color_apply_all',imgType=3,action=paramToggle,param=-1006})
Element:new(_colAdjBox, {x=99,y=336,w=200,h=30,text={str='Also affect project Custom Colors',style=2,align=4,col={180,180,181}}})
Button:new(_colAdjBox, {flow=false,x=290,y=336,w=30,img='bin',imgType=3,action=resetColorControls,helpR=helpR_resetColAdj})
Element:new(_colAdjBox, {x=329,y=336,w=165,h=28,text={str='RESET all Color Controls',style=2,align=4,col={180,180,181}},helpR=helpR_resetColAdj})

Element:new(_colAdjBox, {x=30,y=382,w=450,h=1,color={116,128,149,225}}) -- title div
Element:new(_colAdjBox, {x=0,y=394,w=511,h=20,text={str='Additional settings for AE and BC Series:',style=3,align=5,col={200,200,202}}})

Element:new(_colAdjBox, {x=30,y=426,w=138,h=16,text={str='TRACK NAMES',style=2,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=30,y=444,w=138,h=16,text={str='Indicates Properties',style=2,align=5,col={179,197,229}}})
Spinner:new(_colAdjBox, {x=30,y=466,w=138,title='SHOW',action=paramSet,param='glb_trackLabelCol',valsTable=trackLabelColVals}) --- CSIX

Element:new(_colAdjBox, {x=186,y=426,w=138,h=16,text={str='ACTION BUTTONS',style=2,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=186,y=444,w=138,h=16,text={str='* Label Brightness',style=2,align=5,col={179,197,229}}})
Spinner:new(_colAdjBox, {x=186,y=466,w=138,title='BRIGHT.',action=paramSet,param='ctrl_Param_201',valsTable=buttonBrightVals}) --- CSIX

Element:new(_colAdjBox, {x=342,y=426,w=138,h=16,text={str='TCP/MCP TINT',style=2,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=342,y=444,w=138,h=16,text={str='* Custom Color',style=2,align=5,col={179,197,229}}})
Spinner:new(_colAdjBox, {x=342,y=466,w=138,title='COVERAGE',action=paramSet,param='glb_customOverlay',valsTable=tcpCustomColorVals}) --- CSIX

Element:new(_colAdjBox, {x=30,y=430,w=138,h=70,color={253,253,253,0},helpL=helpL_layout}) -- helpL
Element:new(_colAdjBox, {x=186,y=430,w=138,h=70,color={253,253,253,0},helpL=helpL_layout,helpR=helpR_buttonBrightness}) -- helpL, helpR
Element:new(_colAdjBox, {x=342,y=430,w=138,h=70,color={253,253,253,0},helpL=helpL_layout,helpR=helpR_custOverlay}) -- helpL, helpR

Element:new(_colAdjBox, {x=30,y=516,w=450,h=1,color={116,128,149,225}}) -- title div

Button:new(_colAdjBox, {flow=false,x=158,y=525,w=205,h=20,img='totalReset',imgType=3,action=resetParams,helpR=helpR_totalReset}) --- CSIX totalReset
Button:new(_pageGlobal_und, {flow=false,x=150,y=608,w=205,h=40,img='themelogo',imgType=4}) --- CSIX theme logo

Element:new(_pageGlobal_und, {x=448,y=568,w=60,h=12,text={str='v.3.10',style=2,align=6,col={180,180,181}},helpR=helpR_showTcpParm,helpL=helpL_layout}) --- CSIX
...
Spinner:new(_pageTrack_und, {x=348,y=6,w=155,title='HIDE ACTIONS *',action=paramSet,param='tcp_customButton',valsTable=tcpCustomButtonVals,helpL=helpL_layout,helpR=helpR_showActBtnTcp}) --- CSIX

Button:new(_pageTrack_und, {x=261,y=39,w=30,img='Check-buttons_check',imgType=3,action=actionToggle,param=40302,helpR=helpR_showTcpFx,helpL=help_pref}) --- CSIX
...
Button:new(_pageTrack_und, {flow=false,x=303,y=39,w=30,img='Check-buttons_check',imgType=3,action=paramToggle,param='tcp_fxParmVis',helpR=helpR_showTcpParm,helpL=helpL_layout}) --- CSIX
...
Button:new(_pageTrack_und, {x=442,y=39,w=30,img='Check-buttons_check',imgType=3,action=actionToggle,param=40677,helpR=helpR_showTcpSends,helpL=help_pref}) --- CSIX
...
_tcpButLayA = Button:new(_pageTrack_und, {x=20,y=65,w=69,img='layout_docked_A',imgType=3,action=doActiveLayout,param={'tcp','A'},helpR=helpR_layoutButton})
_tcpButLayB = Button:new(_pageTrack_und, {x=93,y=65,w=69,img='layout_docked_B',imgType=3,action=doActiveLayout,param={'tcp','B'},helpR=helpR_layoutButton})
_tcpButLayC = Button:new(_pageTrack_und, {x=166,y=65,w=69,img='layout_docked_C',imgType=3,action=doActiveLayout,param={'tcp','C'},helpR=helpR_layoutButton})
Readout:new(_tcpButLayA, {x=7,y=50,w=60,h=12,text={str='Default',style=1,align=5,colFalse={254,254,254,30},colTrue={202,202,202}},updateState=isDefault,getParam={'tcp','A'},helpR=helpR_default})
Readout:new(_tcpButLayB, {x=7,y=50,w=60,h=12,text={str='Default',style=1,align=5,colFalse={254,254,254,30},colTrue={202,202,202}},updateState=isDefault,getParam={'tcp','B'},helpR=helpR_default})
Readout:new(_tcpButLayC, {x=7,y=50,w=60,h=12,text={str='Default',style=1,align=5,colFalse={254,254,254,30},colTrue={202,202,202}},updateState=isDefault,getParam={'tcp','C'},helpR=helpR_default})
Readout:new(_tcpButLayA, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,30},colTrue={255,128,4}},updateState=anySelected,getParam={'tcp','A'},helpR=helpR_selected})
Readout:new(_tcpButLayB, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,30},colTrue={255,128,4}},updateState=anySelected,getParam={'tcp','B'},helpR=helpR_selected})
Readout:new(_tcpButLayC, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,30},colTrue={255,128,4}},updateState=anySelected,getParam={'tcp','C'},helpR=helpR_selected})

_applyStroke_Track = Element:new(_pageTrack_und, {x=254,y=70,w=259,h=83,color={116,128,149,225}}) -- stroke
_applyBox_Track = Element:new(_applyStroke_Track, {x=1,y=1,w=257,h=83,color={58,60,64},helpL=helpL_applySize}) -- fill
...
apply.und.tcp[2] = Button:new(_applyBox_Track, {x=1090,y=25,w=61,img='apply_150',imgType=3,action=applyLayout,param={'tcp','150%_'},helpR=helpR_applySize,helpL=helpL_applySize}) --- CSIX (NA)
...
--- CSIX (no left meters)
_layoutTrack = Element:new(_layoutTrackStroke, {x=1,y=1,w=511,h=494,color={58,60,64},helpL=helpL_layout}) -- fill
Spinner:new(_layoutTrack, {x=30,y=9,w=129,title='NAME SIZE *',action=paramSet,param='tcp_LabelSize',valsTable=tcpLabelVals,helpL=helpL_layout,helpR=helpR_LabelSize}) --- CSIX (global name size)
Spinner:new(_layoutTrack, {x=30,y=45,w=129,title='VOLUME SIZE',action=paramSet,param='tcp_vol_size',valsTable=tcpVolVals})
Spinner:new(_layoutTrack, {x=30,y=82,w=129,title='GUIDES *',action=paramSet,param='tcp_indentGuide',valsTable=indentGuideVals,helpL=helpL_layout,helpR=helpR_showIndentGuide}) --- CSIX (indentGuide)
Spinner:new(_layoutTrack, {x=190,y=9,w=129,title='INPUT SIZE',action=paramSet,param='tcp_InputSize',valsTable=tcpInVals})
Spinner:new(_layoutTrack, {x=190,y=45,w=129,title='METER SIZE',action=paramSet,param='tcp_MeterSize',valsTable=tcpMeterVals}) --- CSIX (metersize)
Spinner:new(_layoutTrack, {x=190,y=82,w=129,title='not used'}) --- CSIX (not used)
--- tcpMeterLocVals = {'LEFT','RIGHT','LEFT IF ARMED'} --- CSIX (no left meters)
--- Spinner:new(_layoutTrack, {x=273,y=70,w=159,title='METER LOCATION',action=paramSet,param='tcp_MeterLoc',valsTable=tcpMeterLocVals}) --- CSIX (no left meters)
Spinner:new(_layoutTrack, {x=353,y=9,w=129,title='SEP. SENDS',action=paramSet,param='tcp_sepSends',valsTable=seperateSendsVals,helpL=helpL_layout}) --- CSIX (sendsList)
Spinner:new(_layoutTrack, {x=353,y=45,w=129,title='METER FLIP *',action=paramSet,param='tcp_meterFlip',valsTable=meterFlipVals,helpR=helpR_showActVuflip,helpL=helpL_layout}) --- CSIX (meterFlip)
Spinner:new(_layoutTrack, {x=353,y=82,w=129,title='FX-LIST *',action=paramSet,param='tcp_fxparm_column',valsTable=tcpFxListColumnVal,helpR=helpR_fxparmColumns,helpL=helpL_layout}) --- CSIX (fx list)
...
Spinner:new(_pageMixer_und, {x=348,y=6,w=155,title='MCP ACTIONS *',action=paramSet,param='mcp_customButton',valsTable=mcpCustomButtonVals,helpL=helpL_layout,helpR=helpR_showActBtnMcp}) --- CSIX

_mixerTopStroke = Element:new(_pageMixer_und, {x=0,y=133,w=513,h=330,color={116,128,149,225}}) -- stroke
_mcpButLayA = Button:new(_pageMixer_und, {x=30,y=45,w=69,img='layout_docked_A_blue',imgType=3,action=doActiveLayout,param={'mcp','A'},helpR=helpR_layoutButton})
_mcpButLayB = Button:new(_pageMixer_und, {x=103,y=45,w=69,img='layout_docked_B_blue',imgType=3,action=doActiveLayout,param={'mcp','B'},helpR=helpR_layoutButton})
_mcpButLayC = Button:new(_pageMixer_und, {x=176,y=45,w=69,img='layout_docked_C_blue',imgType=3,action=doActiveLayout,param={'mcp','C'},helpR=helpR_layoutButton})
Readout:new(_mcpButLayA, {x=7,y=50,w=60,h=12,text={str='Default',style=1,align=5,colFalse={254,254,254,30},colTrue={202,202,202}},updateState=isDefault,getParam={'mcp','A'},helpR=helpR_default})
Readout:new(_mcpButLayB, {x=7,y=50,w=60,h=12,text={str='Default',style=1,align=5,colFalse={254,254,254,30},colTrue={202,202,202}},updateState=isDefault,getParam={'mcp','B'},helpR=helpR_default})
Readout:new(_mcpButLayC, {x=7,y=50,w=60,h=12,text={str='Default',style=1,align=5,colFalse={254,254,254,30},colTrue={202,202,202}},updateState=isDefault,getParam={'mcp','C'},helpR=helpR_default})
Readout:new(_mcpButLayA, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,30},colTrue={255,128,4}},updateState=anySelected,getParam={'mcp','A'},helpR=helpR_selected})
Readout:new(_mcpButLayB, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,30},colTrue={255,128,4}},updateState=anySelected,getParam={'mcp','B'},helpR=helpR_selected})
Readout:new(_mcpButLayC, {x=7,y=65,w=60,h=12,text={str='Selected',style=1,align=5,colFalse={254,254,254,30},colTrue={255,128,4}},updateState=anySelected,getParam={'mcp','C'},helpR=helpR_selected})

_applyStroke_Mixer = Element:new(_pageMixer_und, {x=274,y=50,w=239,h=83,color={116,128,149,225},helpL=helpL_applySize}) -- stroke
_applyBox_Mixer = Element:new(_applyStroke_Mixer, {x=1,y=1,w=237,h=83,color={58,60,64},helpL=helpL_applySize}) -- fill
...
apply.und.mcp[2] = Button:new(_applyBox_Mixer, {x=1090,y=25,w=61,img='apply_150',imgType=3,action=applyLayout,param={'mcp','150%_'},helpR=helpR_applySize,helpL=helpL_applySize}) --- CSIX (scale not available) ORG x=90,y=29,w=61
apply.und.mcp[3] = Button:new(_applyBox_Mixer, {x=115,y=25,w=61,img='apply_200',imgType=3,action=applyLayout,param={'mcp','200%_'},helpR=helpR_applySize,helpL=helpL_applySize}) --- CSIX (scale not available) ORG 151,y=29,w=61
Element:new(_applyBox_Mixer, {x=17,y=60,w=200,h=10,img='apply_to_sel'})

_mixerTop = Element:new(_mixerTopStroke, {x=1,y=1,w=511,h=328,color={58,60,64},helpL=helpL_layout}) -- fill
...
Spinner:new(_mixerTop, {x=10,y=48,w=185,title='FX EMBED (SIDEBAR)',action=paramSet,param='mcp_fxEmbedSize',valsTable=mcpFxEmbedVals,helpL=helpL_layout,helpR=helpR_embed}) --- CSIX (mcp fxembed size)
Spinner:new(_mixerTop, {x=10,y=86,w=185,title='FX EMBED (MASTER WS)',action=paramSet,param='mcp_fxEmbedSizeMain',valsTable=mcpFxEmbedMainVals,helpL=helpL_layout,helpR=helpR_embedMast}) --- CSIX (mcp master fxembed size)
...
Spinner:new(_mixerTop, {x=208,y=48,w=140,title='Title-Track Size*',action=paramSet,param='glb_TitleTrackSize',valsTable=mcpTitleTrackSize,helpR=helpR_titleTrack1}) --- CSIX (mcp title-track)
Spinner:new(_mixerTop, {x=208,y=86,w=140,title='Title-Track Col.*',action=paramSet,param='glb_TitleTrackCol',valsTable=mcpTitleTrackCol,helpR=helpR_titleTrack2}) --- CSIX (tcp-mcp title-track)
Spinner:new(_mixerTop, {x=360,y=10,w=140,title='VOL SIZE (C)',action=paramSet,param='mcp_StripVolumeSize',valsTable=mcpStripVolVals}) --- CSIX (strips volume size)
Spinner:new(_mixerTop, {x=360,y=48,w=140,title='NAME SIZE (C)',action=paramSet,param='mcp_StripNameSize',valsTable=mcpStripNameVals}) --- CSIX (strips name size)
Spinner:new(_mixerTop, {x=360,y=86,w=140,title='EXT.MIXER (C) *',action=paramSet,param='mcp_StripExtMix',valsTable=mcpStripExtMixVals,helpL=helpL_layout,helpR=helpR_extmix}) --- CSIX (mcp strips hide extended mixer)
...
_genMixer = Element:new(_mMid, {x=1,y=1,w=511,h=29,color={50,51,55},helpL=help_pref}) -- fill
...
_extMixerf = Element:new(_mLower, {x=1,y=1,w=255,h=143,color={50,51,55},helpL=help_pref}) -- fill
...
_mastMixer = Element:new(_mLower, {x=256,y=1,w=256,h=143,color={58,60,64},helpL=help_pref}) -- fill
Element:new(_mastMixer, {x=30,y=8,w=257,h=20,text={str='MASTER MIXER :',style=2,align=0,col={180,180,181}}})
Button:new(_mastMixer, {x=24,y=21,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=40389})
Element:new(_mastMixer, {x=53,y=21,w=190,h=30,text={str='Show Master on Right Side',style=2,align=4,col={180,180,181}}})
Button:new(_mastMixer, {x=24,y=45,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=41610})
Element:new(_mastMixer, {x=53,y=45,w=190,h=30,text={str='Toggle in Dock',style=2,align=4,col={180,180,181}}})
Button:new(_mastMixer, {x=24,y=67,w=30,img='Check-buttons_check_blue',imgType=3,action=actionToggle,param=41636})
Element:new(_mastMixer, {x=53,y=67,w=190,h=30,text={str='Toggle in Floating Window',style=2,align=4,col={180,180,181}}})
Button:new(_mastMixer, {x=24,y=89,w=30,img='Check-buttons_check_blue',imgType=3,action=paramToggle,param='glb_simpleMaster'})
Element:new(_mastMixer, {x=53,y=89,w=190,h=30,text={str='Always Show Meter Scales *',style=2,align=4,col={180,180,181}},helpR=helpR_mastermeter})
Button:new(_mastMixer, {x=24,y=111,w=30,img='Check-buttons_check_blue',imgType=3,action=paramToggle,param='glb_wideMaster'})
Element:new(_mastMixer, {x=53,y=111,w=190,h=30,text={str='Wide Meter (for AE Master W) *',style=2,align=4,col={180,180,181}},helpR=helpR_masterw})
...
_paletteBox = Element:new(_paletteBoxStroke, {x=1,y=1,w=511,h=225,color={58,60,64},helpL=helpL_customCol}) -- fill

Readout:new(_paletteBox, {x=124,y=9,w=263,h=30,param={'scriptVariable',palette.current},text={str='REAPER V6',style=4,align=5,col={180,180,181}},valsTable=undockPaletteNamesVals})
...
Element:new(_paletteBox, {x=30,y=102,w=450,h=30,text={str='Click a Color to Apply it to all Selected Tracks  (+ Ctrl/Cmd for Items)',style=3,align=5,col={180,180,181}}})

--- Set items to default color, extra button
Button:new(_paletteBox, {x=28,y=140,w=30,img='color_apply_all',imgType=3,action=applyPalette,helpR=helpR_recolProject})
Element:new(_paletteBox, {x=67,y=140,w=208,h=30,text={str='Project: Recolor using this Palette *',style=2,align=4,col={180,180,181}},helpR=helpR_recolProject})

Button:new(_paletteBox, {x=68,y=180,w=30,img='color_apply_all',imgType=3,action=applyChildren,helpR=helpR_colgradeChildren})
Element:new(_paletteBox, {x=107,y=180,w=208,h=30,text={str='Color Gradient to Children *',style=2,align=4,col={180,180,181}},helpR=helpR_colgradeChildren})

--- Set items to default color
Button:new(_paletteBox, {x=280,y=140,w=30,img='bin',imgType=3,action=setItemsDefaultColor,helpR=helpR_recolProjec})
Element:new(_paletteBox, {x=315,y=140,w=208,h=30,text={str='Items: Set to Default Color',style=2,align=4,col={180,180,181}}})

--- Set takes to default color
Button:new(_paletteBox, {x=280,y=180,w=30,img='bin',imgType=3,action=setTakesDefaultColor,helpR=helpR_recolProjec})
Element:new(_paletteBox, {x=315,y=180,w=208,h=30,text={str='Takes: Set to Default Color',style=2,align=4,col={180,180,181}}})

_paletteMenuBox = Element:new(_paletteBoxStroke, {x=1,y=222,w=511,h=272,color={50,51,55}}) -- fill
...
Button:new(_pageColor_und, {x=28,y=507,img='color_dim_all',imgType=3,action=reduceCustCol,param=false,helpR=helpL_colDimming})
Element:new(_pageColor_und, {x=67,y=507,w=208,h=30,text={str='Dim all Assigned custom colors',style=2,align=4,col={180,180,181}},helpR=helpL_colDimming})

Button:new(_pageColor_und, {x=28,y=545,w=30,img='color_dim_all',imgType=3,action=reduceCustCol,param=true,helpR=helpL_colDimming})
Element:new(_pageColor_und, {x=67,y=545,w=208,h=30,text={str='Dim custom colors on Selected tracks',style=2,align=4,col={180,180,181}},helpR=helpL_colDimming})

Button:new(_pageColor_und, {x=28,y=584,w=30,img='bin',imgType=3,action=setTrackDefaultColor})
Element:new(_pageColor_und, {x=67,y=583,w=208,h=30,text={str='RESET selected Tracks to Default color',style=2,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=278,y=507,w=30,img='color_dim',imgType=3,action=TrackRandomColor})
Element:new(_pageColor_und, {x=315,y=507,w=208,h=30,text={str='Track: Set to Random colors (native)',style=2,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=278,y=545,w=30,img='color_dim',imgType=3,action=TrackCustomColor})
Element:new(_pageColor_und, {x=315,y=545,w=208,h=30,text={str='Track: Set to Custom color (picker)',style=2,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=278,y=583,w=30,img='toolbar_colorpal',imgType=3,action=CustomColorScript})
Element:new(_pageColor_und, {x=315,y=583,w=208,h=30,text={str='Rodilab Color palette (ReaPack add-on)',style=2,align=4,col={180,180,181}}})
...
apply.und.envcp[2] = Button:new(_pageEnvTrans_und, {x=1228,y=2,w=61,img='apply_150',imgType=3,action=applyLayout,param={'envcp','150%_'},helpL=helpL_applySize}) --- CSIX (scale not available) ORG x=228,y=2,w=61
...
Button:new(_pageEnvTrans_und, {flow=false,x=150,y=104,w=30,img='Check-buttons_check_purple',imgType=3,action=paramToggle,param='envcp_folder_indent',helpR=helpR_emvMatchIndent}) --- CSIX
Element:new(_pageEnvTrans_und, {x=178,y=104,w=180,h=30,text={str='MATCH TRACK FOLDER INDENT',style=2,align=4,col={180,180,181}},helpR=helpR_emvMatchIndent})

Element:new(_pageEnvTrans_und, {x=153,y=222,w=200,h=23,img='transport_title'})
...
apply.und.trans[2] = Button:new(_pageEnvTrans_und, {x=1228,y=278,w=61,img='apply_150',imgType=3,action=applyLayout,param={'trans','150%_'},helpL=helpL_applySize}) --- CSIX (scale not available) ORG x=228,y=2,w=61
...
Element:new(_pageEnvTrans_und, {x=154,y=322,w=200,h=16,text={str='* See Help text ->',style=2,align=5,col={179,197,229}}}) --- CSIX
Spinner:new(_pageEnvTrans_und, {x=10,y=352,w=149,title='ACTIONS *',action=paramSet,param='trans_actionButton',valsTable=transActBtnVals,helpR=helpR_showActBtnTrans,helpL=helpL_layout}) --- CSIX
Spinner:new(_pageEnvTrans_und, {x=180,y=352,w=149,title='CUSTOM BTNS *',action=paramSet,param='trans_metroButton',valsTable=transMetroBtnVals,helpR=helpR_showMetroBtnTrans,helpL=helpL_layout}) --- CSIX
Spinner:new(_pageEnvTrans_und, {x=348,y=352,w=149,title='PLAY RATE SIZE *',action=paramSet,param='trans_rate_size',valsTable=transRateVals,helpR=help_playRate,helpL=helpL_layout})

_transPrefsStroke = Element:new(_pageEnvTrans_und, {x=0,y=415,w=513,h=176,color={116,128,149,225}}) -- stroke
_transPrefs = Element:new(_transPrefsStroke, {x=1,y=1,w=511,h=174,color={50,51,55},helpL=help_pref}) -- fill

Button:new(_transPrefs, {x=29,y=26,img='Check-buttons_check_purple',imgType=3,action=actionToggle,param=40533}) --- CSIX
Element:new(_transPrefs, {x=68,y=26,w=150,h=30,text={str='Center Transport',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=29,y=69,img='Check-buttons_check_purple',imgType=3,action=actionToggle,param=40531}) --- CSIX
Element:new(_transPrefs, {x=68,y=69,w=150,h=30,text={str='Show Play Rate',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=29,y=112,img='Check-buttons_check_purple',imgType=3,action=actionToggle,param=40680}) --- CSIX
Element:new(_transPrefs, {x=68,y=112,w=170,h=30,text={str='Show Time Signature',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=26,img='Check-buttons_check_purple',imgType=3,action=actionToggle,param=40868}) --- CSIX
Element:new(_transPrefs, {x=324,y=26,w=170,h=30,text={str='Use Home/End for Markers',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=69,img='Check-buttons_check_purple',imgType=3,action=actionToggle,param=40532}) --- CSIX
Element:new(_transPrefs, {x=324,y=69,w=170,h=30,text={str='Show Play State as Text',style=2,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=112,img='Check-buttons_check_purple',imgType=3,action=actionToggle,param=40620}) --- CSIX
Element:new(_transPrefs, {x=324,y=112,w=170,h=30,text={str='External Timecode Sync.',style=2,align=4,col={180,180,181}}})

--HELP
_helpL = Element:new(_pageContainer, {x=-144,y=500,w=115,h=200,text={str='',style=2,align=2,wrap=true,vCenter=false,lineSpacing=14,col={180,180,181}}})
Element:new(_helpL, {x=18,y=-25,w=97,h=19,img='helpHeader_l'})
_helpR = Element:new(_pageContainer, {x=542,y=200,w=115,h=200,text={str='Show play state',style=2,align=0,wrap=true,vCenter=false,lineSpacing=14,col={180,180,181}}})
